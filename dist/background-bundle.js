...
/***/ function(module, exports, __webpack_require__) {
	...
	var currentSeries, currentSeason, currentEpisode;
	var currPageIndex = 0;

	var BASE_URL = 'http://primewire.ag';

	(0, _localStorageHelper.retrieve)('TheNextEpisode').then(function (result) {
		// set initial series and season
		if (result && result.length) {
			currentSeries = _lodash2.default.first(result);
			currentSeason = _lodash2.default.first(currentSeries.seasons);
		} else {
			createStorageObject();
		}
	});
	...
	var createStorageObject = function createStorageObject() {
		(0, _localStorageHelper.store)({ 'TheNextEpisode': [] }).then(function () {
			console.log("Series array created in local storage.");
		});
	};

	// get the various host links for the episode
	var pruneLinks = function pruneLinks(html) {
		var availableLinks = (0, _jquery2.default)(".movie_version", html);
		var prunedLinks = [];

		for (i = 0; i < availableLinks.length; i++) {
			var x = availableLinks[i];
			var website = (0, _jquery2.default)(x).find('.version_host').text().split("'")[1];
			...
			prunedLinks.push({
				website: website,
				link: (0, _jquery2.default)(x).find('.movie_version_link a').attr('href'),
				rating: (0, _jquery2.default)(x).find('.current-rating').text().split(' ')[1],
				num_views: (0, _jquery2.default)(x).find('.version_veiws').text().split(' ')[1]
			});
		}
	...
	var fetchEpisodeDetails = function fetchEpisodeDetails(episode, callback) {
		var urlToFetch = '' + BASE_URL + episode.url;
		return (0, _isomorphicFetch2.default)(urlToFetch).then(function (html) {
	...
	// helper functions
	var getCurrentSeriesIndex = function getCurrentSeriesIndex() {
		return currentSeries.indexOfSeries(currentSeason[0].seriesName);
	};

	var getCurrentSeries = function getCurrentSeries() {
	...
	Array.prototype.getEpisodeByName = function (episodeName) {
		for (var i = 0; i < this.length; i++) {
			if (this[i].episodeName == episodeName) return this[i];
		}
		return -1;
	};
	...
	Array.prototype.indexOfEpisode = function (episodeName) {
		for (var i = 0; i < this.length; i++) {
			if (this[i].episodeName == episodeName) return i;
		}
		return -1;
	};
	...
	Array.prototype.indexOfFirstEpOfSeason = function (num) {
		for (var i = 0; i < this.length; i++) {
			if (this[i].seasonNumber == num) return i;
		}return -1;
	};
